...
function InfoRow({ label, value, isMatched }) {
  return (
    <div className="flex justify-between gap-4 border-b border-slate-100 dark:border-slate-800/40 pb-2">
      ...
      <span className="font-extrabold text-slate-800 dark:text-slate-205 text-sm text-right flex items-center gap-1 justify-end">
        {normalizeText(value)}
        {isMatched && (
...
function PipelineNode({ label, value, isMatched, lang, icon }) {
  return (
    <div className={`p-3.5 rounded-2xl border transition-all duration-300 flex items-center justify-between gap-3 ${
      isMatched 
        ? "bg-emerald-500/[0.03] dark:bg-emerald-500/[0.02] border-emerald-200/80 dark:border-emerald-800/40 hover:border-emerald-350 dark:hover:border-emerald-700/60 shadow-sm"
        : "bg-slate-50/50 dark:bg-slate-900/10 border-slate-200 dark:border-slate-800 hover:border-indigo-150 dark:hover:border-indigo-900/40"
    }`}>
      <div className="flex items-center gap-3 overflow-hidden">
        ...
        <div className="overflow-hidden">
          ...
          <p className="text-sm font-black text-slate-850 dark:text-slate-150 mt-0.5 truncate">{value === "N/A" || !value ? (lang === "VI" ? "Không có" : "None") : String(value)}</p>
        </div>
      </div>
...
function AgentCard({ agentKey, title, method, data, finalDenomination, finalCountry, finalMaterial, t }) {
  ...
  if (!data) {
    return (
      <div className="bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 shadow-sm rounded-3xl p-6 flex flex-col h-full hover:border-slate-350 dark:hover:border-slate-700 transition duration-300">
        <span className="inline-block px-2 py-0.5 bg-slate-100 dark:bg-slate-850 border border-slate-205 dark:border-slate-755 text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-wider rounded mb-3 w-fit">
          {agentKey}
        </span>
  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200 dark:border-slate-800/80 shadow-md rounded-[2rem] p-6 flex flex-col hover:border-indigo-350 dark:hover:border-indigo-800/60 hover:shadow-xl transition duration-300">
      <div className="flex justify-between items-start gap-4 mb-4">
        <div>
          <span className="inline-block px-2 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-750 text-slate-500 dark:text-slate-450 text-[10px] font-black uppercase tracking-wider rounded mb-2 w-fit">
            {agentKey}
          </span>
      ...
      <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800/50">
        ...
        <div
          className={`text-xs text-slate-655 dark:text-slate-350 bg-slate-50/50 dark:bg-slate-950/40 border border-slate-205 dark:border-slate-800/80 p-4 rounded-2xl leading-relaxed ${
        ...
        {reasoningText.length > 150 && (
          <button
            ...
            className="mt-3 text-indigo-650 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 text-xs font-black transition-colors flex items-center gap-1 cursor-pointer"
          >
            {isExpanded ? (
              <>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200 dark:border-slate-800 shadow-md rounded-[2rem] p-6 hover:shadow-xl hover:border-slate-300 dark:hover:border-slate-700 transition duration-300">
        ...
        <p className="text-sm text-slate-550 dark:text-slate-400 mt-1 mb-5">{t.aggDesc}</p>
          ...
          <div className="col-span-1 md:col-span-5 z-10 h-full flex items-center">
            <div className="w-full p-5 rounded-[1.5rem] bg-indigo-500/[0.04] dark:bg-indigo-950/20 border border-indigo-200/70 dark:border-indigo-900/40 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] font-black uppercase tracking-wider text-indigo-650 dark:text-indigo-400">Aggregator (Trọng tài)</p>
              </div>
              ...
              <p className="text-xs text-slate-550 dark:text-slate-400 mt-1.5 leading-relaxed font-semibold">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <AgentCard agentKey="Agent 3" title="Visual Search" method={lang === "VI" ? "Đối chiếu hình ảnh ngoài" : "External matching"} data={agents.visual_search} finalDenomination={finalDenomination} finalCountry={finalCountry} finalMaterial={finalMaterial} t={t} />
        </div>
      </div>
    </div>
  );
