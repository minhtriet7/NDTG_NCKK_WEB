...
header_content = """import { useState } from "react";
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`group relative flex items-center gap-2 px-3.5 py-2 text-sm font-medium transition-all duration-300 rounded-xl
                      ${
                        active
                  ...
                  >
                    {link.icon && (
                      <link.icon className={`w-4 h-4 transition-colors duration-300 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 group-hover:text-indigo-500 dark:group-hover:text-indigo-400'}`} />
                    )}
                    {link.label}
                    {active && (
                      <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-[3px] bg-gradient-to-r from-indigo-500 to-purple-500 rounded-t-full shadow-[0_-2px_10px_rgba(99,102,241,0.5)]" />
                    )}
                  </Link>
                );
              })}
...
with open("client/src/components/layout/Header.jsx", "w", encoding="utf-8") as f:
